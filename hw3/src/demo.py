        ...
        if seq.no_gt:
            ...
            if verbose:
                ...
                for index, event in reid_events.iterrows():
                    frame_id, _ = index
                    match_events_oid = match_events[
                        match_events["OId"] == event["OId"]
                    ]
                    match_events_oid_earlier = match_events_oid[
                        match_events_oid.index.get_level_values("FrameId")
                        < frame_id
                    ]
                    ...
                    if not match_events_oid_earlier.empty:
                        ...
                        last_occurrence = (
                            match_events_oid_earlier_frame_ids.max()
                        )
                ...
                if switch_gaps:
                    switch_gaps_hist, _ = np.histogram(
                        switch_gaps,
                        bins=list(range(0, max(switch_gaps) + 10, 10)),
                    )
                    switch_gaps_hist = switch_gaps_hist.tolist()

                _log.info(
                    f"SWITCH_GAPS_HIST (bin_width=10): {switch_gaps_hist}"
                )
