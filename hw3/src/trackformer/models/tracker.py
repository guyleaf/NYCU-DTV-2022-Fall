...
class Tracker:
    ...
    def __init__(
        ...
        self.detection_obj_score_thresh = tracker_cfg[
            "detection_obj_score_thresh"
        ]
        ...
        if self.generate_attention_maps:
            assert hasattr(
                self.obj_detector.transformer.decoder.layers[-1],
                "multihead_attn",
            ), "Generation of attention maps not possible for deformable DETR."

            attention_data = {"maps": None, "conv_features": {}, "hooks": []}

            hook = self.obj_detector.backbone[-2].register_forward_hook(
                lambda self, input, output: attention_data.update(
            ...
            def add_attention_map_to_data(self, input, output):
                height, width = attention_data["conv_features"][
                    "3"
                ].tensors.shape[-2:]
            ...
            multihead_attn = self.obj_detector.transformer.decoder.layers[
                -1
            ].multihead_attn
        ...
        if new_track_ids:
            ...
            if aux_results is not None:
                ...
                for new_track_id, aux_score in zip(new_track_ids, aux_scores):
                    self._logger(
                        f"AUX SCORES ID {new_track_id}: {[f'{s:.2f}' for s in aux_score]}"
                    )
    ...
    def public_detections_mask(self, new_det_boxes, public_det_boxes):
        ...
        public_detections_mask = (
            torch.zeros(new_det_boxes.size(0)).bool().to(self.device)
        )
        ...
        if self.public_detections == "center_distance":
            ...
            new_det_boxes_cxcy = (
                box_xyxy_to_cxcywh(new_det_boxes).cpu().numpy()[:, :2]
            )
            public_det_boxes_cxcy = (
                box_xyxy_to_cxcywh(public_det_boxes).cpu().numpy()[:, :2]
            )

            dist3 = new_det_boxes_cxcy.reshape(
                -1, 1, 2
            ) - public_det_boxes_cxcy.reshape(1, -1, 2)
            ...
            iou_matrix = box_iou(
                new_det_boxes, public_det_boxes.to(self.device)
            )
    ...
    def reid(
        ...
        if self.reid_greedy_matching:
            new_det_boxes_cxcyhw = (
                box_xyxy_to_cxcywh(new_det_boxes).cpu().numpy()
            )
            inactive_boxes_cxcyhw = (
                box_xyxy_to_cxcywh(
                    torch.stack([track.pos for track in self.inactive_tracks])
                )
                .cpu()
                .numpy()
            )

            dist_mat = inactive_boxes_cxcyhw[:, :2].reshape(
                -1, 1, 2
            ) - new_det_boxes_cxcyhw[:, :2].reshape(1, -1, 2)
            dist_mat = (dist_mat**2).sum(axis=2)

            track_size = (
                inactive_boxes_cxcyhw[:, 2] * inactive_boxes_cxcyhw[:, 3]
            )
    ...
    def step(self, blob):
        ...
        if self.inactive_tracks:
            self._logger(
                f"INACTIVE TRACK IDS: {[t.id for t in self.inactive_tracks]}"
            )
        ...
        if num_prev_track:
            track_query_boxes = torch.stack(
                [t.pos for t in self.tracks + self.inactive_tracks], dim=0
            ).cpu()
            ...
            target["track_query_hs_embeds"] = torch.stack(
                [t.hs_embed[-1] for t in self.tracks + self.inactive_tracks],
                dim=0,
            )
        ...
        outputs, _, features, _, _ = self.obj_detector(
            img, target, self._prev_features[0]
        )
        ...
        if num_prev_track:
            ...
            if self.generate_attention_maps:
                track_attention_maps = self.attention_data["maps"][
            ...
            for i, track in enumerate(
                self.inactive_tracks, start=len(self.tracks)
            ):
                if track_keep[i]:
                    track.score = track_scores[i]
                    track.hs_embed.append(hs_embeds[i])
        ...
        if self.generate_attention_maps:
            new_det_attention_maps = self.attention_data["maps"][
                -self.num_object_queries :
            ]
        ...
        if self.generate_attention_maps:
            new_det_attention_maps = new_det_attention_maps[
                public_detections_mask
            ]
        ...
        if self.detection_nms_thresh and self.tracks:
            ...
            new_track_mask = torch.tensor(
                [True if t.id in new_track_ids else False for t in self.tracks]
            )
            track_scores[~new_track_mask] = np.inf

            keep = nms(track_boxes, track_scores, self.detection_nms_thresh)
            remove_tracks = [
                track for i, track in enumerate(self.tracks) if i not in keep
            ]
            ...
            self.tracks = [
                track for track in self.tracks if track not in remove_tracks
            ]
        ...
        if "masks" in result and self.tracks:
            track_mask_probs = torch.stack(
                [track.mask for track in self.tracks]
            )
        ...
        for track in self.tracks:
            ...
            if self.obj_detector.overflow_boxes:
                results[track.id]["bbox"] = track.pos.cpu().numpy()
                # self.results[track.id][self.frame_index]['bbox'] = track.pos.cpu().numpy()
            else:
                results[track.id]["bbox"] = (
                    clip_boxes_to_image(track.pos, orig_size[0]).cpu().numpy()
                )
                # self.results[track.id][self.frame_index]['bbox'] = clip_boxes_to_image(track.pos, orig_size[0]).cpu().numpy()

            results[track.id]["score"] = track.score.cpu().numpy()
            results[track.id]["obj_ind"] = track.obj_ind.cpu().item()
            ...
            if track.mask is not None:
                results[track.id]["mask"] = track.mask.cpu().numpy()
