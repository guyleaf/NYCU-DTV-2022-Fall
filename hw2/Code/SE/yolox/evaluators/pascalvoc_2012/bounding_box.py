...
class BoundingBox:
    def __init__(
        ...
        if bbType == utils.BBType.Detected and classConfidence is None:
            raise IOError(
        ...
        if typeCoordinates == utils.CoordinatesType.Relative:
            (
                self._x,
                self._y,
                self._w,
                self._h,
            ) = utils.convertToAbsoluteValues(imgSize, (x, y, w, h))
            self._width_img = imgSize[0]
            self._height_img = imgSize[1]
            if format == utils.BBFormat.XYWH:
                self._x2 = self._w
                self._y2 = self._h
    ...
    def getAbsoluteBoundingBox(self, format=utils.BBFormat.XYWH):
        if format == utils.BBFormat.XYWH:
            return (self._x, self._y, self._w, self._h)
        elif format == utils.BBFormat.XYX2Y2:
    ...
    def getRelativeBoundingBox(self, imgSize=None):
        ...
        if imgSize is not None:
            return utils.convertToRelativeValues(
                (imgSize[0], imgSize[1]),
    ...
    def clone(boundingBox):
        absBB = boundingBox.getAbsoluteBoundingBox(format=utils.BBFormat.XYWH)
        # return (self._x,self._y,self._x2,self._y2)
