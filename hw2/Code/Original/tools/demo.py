...
def make_parser():
    ...
    parser.add_argument(
        "--nms", default=0.3, type=float, help="test nms threshold"
    )
    parser.add_argument(
...
def export_outputs(filename: str, outputs: torch.Tensor):
    dtypes = {
        "class": "int32",
        "x_min": "int32",
        "y_min": "int32",
    ...
    data.to_csv(filename, sep=" ", index=False, header=False, encoding="utf-8")
...
def image_demo(predictor, vis_folder, path, current_time, save_result):
    ...
    for image_name in files:
        ...
        result_image, outputs = predictor.visual(
        ...
        if save_result:
            ...
            logger.info(
                "Saving image with detection results in {}".format(
                    save_file_name
                )
            )
            ...
            logger.info(
                "Saving detection results in {}".format(save_file_name)
            )
            export_outputs(save_file_name, outputs)
        ch = cv2.waitKey(0)
...
def main(exp, args):
    ...
    if args.demo == "image":
        image_demo(
            predictor, vis_folder, args.path, current_time, args.save_result
        )
