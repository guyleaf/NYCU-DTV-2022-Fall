...
class Evaluator:
    def GetPascalVOCMetrics(
        ...
        IOUThreshold=0.5,
        method=utils.MethodAveragePrecision.EveryPointInterpolation,
    ):
        ...
        for bb in boundingboxes.getBoundingBoxes():
            ...
            if bb.getBBType() == utils.BBType.GroundTruth:
                groundTruths.append(
                    [
                        bb.getImageName(),
                        bb.getClassId(),
                        1,
                        bb.getAbsoluteBoundingBox(utils.BBFormat.XYX2Y2),
                    ]
                )
            else:
                detections.append(
        ...
        for c in classes:
            ...
            if method == utils.MethodAveragePrecision.EveryPointInterpolation:
    ...
    def PlotPrecisionRecallCurve(
        ...
        for result in results:
            ...
            if showInterpolatedPrecision:
                if (
                    method
                    == utils.MethodAveragePrecision.EveryPointInterpolation
                ):
                    plt.plot(
                        mrec,
                        mpre,
                    ...
                    method
                    == utils.MethodAveragePrecision.ElevenPointInterpolation
                ):
    ...
    def _getAllIOUs(reference, detections):
        ret = []
        bbReference = reference.getAbsoluteBoundingBox(utils.BBFormat.XYX2Y2)
        # img = np.zeros((200,200,3), np.uint8)
        for d in detections:
            bb = d.getAbsoluteBoundingBox(utils.BBFormat.XYX2Y2)
